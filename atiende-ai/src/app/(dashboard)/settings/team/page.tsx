...
export default function TeamPage() {
  const[staff,setStaff]=useState<{id:string;name:string;role:string;speciality:string;_new?:boolean;[k:string]:unknown}[]>([]);const[tid,setTid]=useState('');
  useEffect(()=>{(async()=>{const s=createClient();const{data:{user}}=await s.auth.getUser();if(!user)return;const{data:t}=await s.from('tenants').select('id').eq('user_id',user.id).single();if(!t)return;setTid(t.id);const{data}=await s.from('staff').select('*').eq('tenant_id',t.id).order('name');setStaff(data||[]);})();},[]);

...
const C:Record<string,string>={pending:'bg-yellow-100 text-yellow-700',confirmed:'bg-emerald-100 text-emerald-700',preparing:'bg-orange-100 text-orange-700',ready:'bg-green-100 text-green-700',delivered:'bg-gray-100 text-gray-600',cancelled:'bg-red-100 text-red-700'};
...
export function OrdersList({orders}:{orders:Order[]}) {
  return(<div className="space-y-2">{orders.map(o=>(<Card key={o.id} className="p-4">
    ...
    <div className="text-sm text-gray-600">{(o.items)?.map((it:OrderItem,i:number)=><span key={i}>{it.qty}x {it.name}{i<(o.items?.length ?? 0)-1?', ':''}</span>)}</div>
  </Card>))}</div>);
}

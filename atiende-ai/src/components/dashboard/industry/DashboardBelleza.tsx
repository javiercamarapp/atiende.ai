...
export function DashboardBelleza({ todayData, monthData, appointments, conversations }: IndustryDashProps) {
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Card className="p-6 border-zinc-200/60 shadow-sm"><p className="text-[11px] uppercase tracking-widest text-zinc-400 mb-1">Citas de hoy</p><p className="text-4xl font-bold text-zinc-900 tabular-nums">{appointments.length}</p><p className="text-xs text-zinc-400 mt-1">servicios agendados</p></Card>
        <Card className="p-6 border-zinc-200/60 shadow-sm"><p className="text-[11px] uppercase tracking-widest text-zinc-400 mb-1">Citas este mes</p><p className="text-4xl font-bold text-zinc-900 tabular-nums">{tApts}</p><p className="text-xs text-zinc-400 mt-1">servicios completados</p></Card>
      </div>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[{l:'Mensajes hoy',v:todayData?.messages_inbound||0,I:MessageSquare},{l:'Citas hoy',v:todayData?.appointments_booked||0,I:Calendar},{l:'No-shows mes',v:tNoShow,I:TrendingDown},{l:'Citas mes',v:tApts,I:Calendar}].map(k=>(
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card className="border-zinc-200/60 shadow-sm p-4"><h3 className="text-sm font-medium text-zinc-900 flex items-center gap-2 mb-3"><Calendar className="w-4 h-4 text-zinc-400"/>Agenda de hoy</h3>
